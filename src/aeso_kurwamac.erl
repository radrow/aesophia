...
constr_letfun(Env0, {letfun, Ann, {id, _, Name}, Args, RetT, Body}, S) ->
    ...
    S2 = [ {subtype, Env1, GlobalFunType, DepSelfT}
...
constr_expr(_Env, Expr = {CmpOp, Ann}, {fun_t, _, _, [OpLT, OpRT], RetT = {id, _, "int"}}, S)
  when CmpOp =:= '+' orelse
       CmpOp =:= '*' orelse
       CmpOp =:= '-' orelse
       CmpOp =:= '/' orelse
...
inst_pred_int(Scope) ->
    lists:concat(
      ...
      , [ Q || {Var, {refined_t, _, {id, _, "int"}, _}} <- Scope,
...
solve(_Env, Cs) ->
    ...
    maps:map(fun
                 (_K, V = #kinfo{env = KEnv, base = Base, curr_qs = Qs}) ->
                     V#kinfo{
                       curr_qs =
                           simplify_pred(
