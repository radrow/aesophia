...
bind_ast_funs(TCEnv, AST, Env) ->
    lists:foldr(
      fun({HEAD, _, Con, Defs}, Env0)
            when HEAD =:= namespace orelse HEAD =:= contract
                 orelse HEAD =:= contract_main orelse HEAD =:= contract_interface ->
              ...
              Env2 = bind_funs(
                       [ begin
                             ...
                             DepArgsT =
                                 [ {dep_arg_t, aeso_syntax:get_ann(Arg),
                                    Arg, fresh_liquid(Env, contravariant, name(Arg), ArgT)}
...
has_assumptions(covariant, {refined_t, _, _, [_|_]}) ->
    true;
...
fresh_liquid(_Env, _Variance, _Hint, Type = {refined_t, _, _, _}) ->
    Type;
...
fresh_liquid(Env, Variance, Hint, {app_t, Ann, {id, _, "list"}, [Elem]})
  when Variance =:= contravariant orelse Variance =:= forced_contravariant ->
    {dep_list_t, Ann, fresh_liquid(Env, Variance, Hint, Elem), [?op(nu(), '>=', ?int(0))]};
fresh_liquid(Env, Variance, Hint, {app_t, Ann, {id, _, "list"}, [Elem]}) ->
            ...
            case TDef of
                ...
                {record_t, Fields} ->
                    {dep_record_t, Ann, Type,
                     [ {Field, fresh_liquid(Env, Variance, Hint ++ "_" ++ name(Field),
                                            apply_subst(Subst, FType))}
          ...
          fun() ->
            ...
            Assg ->
                ...
                when ErrType =:= contradict
                     ...
                     orelse ErrType =:= entrypoint_arg_assump
                     -> {error, E}
    end.
...
constr_expr(Env, ?typed_p(Expr, Type = {refined_t, Ann, Base, _}), S0) ->
...
constr_expr(Env, {typed, _, E, T1}, T2, S) ->
    case T1 == T2 of
        ...
        false -> error({internal_type_mismatch, T1, T2})
    end;
...
constr_expr(Env, {list, Ann, Elems}, Type = {app_t, TAnn, _, [ElemT]}, S0) ->
    ...
    {{dep_list_t, TAnn, DepElemT, [?op(nu(), '==', ?int(length(Elems)))]},
     [ {well_formed, Env, DepElemT}
     | S2
     ]
    };
...
match_to_pattern(Env0, {app, _, {'::', _}, [H,T]}, Expr, {dep_list_t, TAnn, ElemT, _}, Pred0) ->
    ...
    match_to_pattern(Env1, T, {id, ann(), "_"},
                     {dep_list_t, TAnn, ElemT, [?op(nu(), '==', ?op(Expr, '-', ?int(1)))]},
                     Pred2).
...
apply_subst(X, Expr, {refined_t, Ann, T, Qual}) ->
    {refined_t, Ann, T, apply_subst(X, Expr, Qual)};
...
inst_pred_variant_tag(Qid, Constrs) ->
    [ {is_tag, ann(), nu(), qid(lists:droplast(qname(Qid)) ++ [name(Con)]), Args}
...
pred_of(Assg, Env = #env{path_pred = PathPred}) ->
    ScopePred = flatten_type_binds(Assg, type_binds(Env)),
    ScopePred ++ PathPred;
pred_of(_, _) ->
    [].

flatten_type_binds(Assg, TB) ->
    flatten_type_binds(Assg, fun(X) -> X end, TB).
        ...
        Q <- case Type of
                 ...
                 {dep_record_t, _, RT, Fields} ->
                     ...
                     flatten_type_binds(
                       Assg,
                       fun(X) -> {proj, ann(), Access(Var), X} end,
                       [{qid(QName ++ [name(Field)]), T} || {Field, T} <- Fields]
                      );
                     ...
                     ConPred =
                         lists:flatten(
                           [ flatten_type_binds(
                               ...
                               fun(X) -> {proj, ann(), Access(Var), X} end,
                               lists:zip(
                                 [?adt_proj_id(qid(lists:droplast(QName) ++ [name(Con)]), I) || I <- lists:seq(1, length(Args))],
                                 Args
                              )
                              )
                 ...
                 {dep_list_t, _, ElemT, LenPred} ->
                     flatten_type_binds(Assg, Access, [ElemT]) ++
                         apply_subst(nu(), Access(Var), pred_of(Assg, LenPred));
                 _ -> []
             end
    ].
            ...
            split_constr(
              ...
              | [ {subtype, Ann, Env0, ArgSub, ArgSup}
                 || { {dep_constr_t, _, _, ArgsSub}
                    , {dep_constr_t, _, _, ArgsSup} %% TODO is_tag path pred?
